...
public class HellFire : MonoBehaviour, IEnemyObserver
{
    ...
    private void Awake()
    {
        _birthTime = Timer.CustomTime;
        _targets = new List<GameObject>();
    }

    private void Update()
    {
        if(Timer.CustomTime - _birthTime > _ability.ActiveTime)
        {
            Destroy(gameObject);
        }    
    ...
    private void OnDestroy()
    {
        ...
        {
            target.GetComponent<EnemyBehavior>().RemoveObserver(this);
        }
    }
