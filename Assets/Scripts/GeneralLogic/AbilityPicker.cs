...
public class AbilityPicker : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    private void RollAbility() //TODO: Add payment check in wallet, not in some external classes
    {
        if (_currentlyAvailableAbilities.Count != 0 && _playerWallet.Gold >= _economySettingsSO.RollAbilityCostGold)
        {
            int abilityIndex = Random.Range(0, _currentlyAvailableAbilities.Count);
            _ability = _currentlyAvailableAbilities[abilityIndex];
            _ability.InitializeAbility();
    ...
    public void RemoveAbility() //TODO: Add payment check in wallet, not in some external classes
    {
        if (_ability)
    ...
    public void UpgradeAbility() //TODO: Add payment check in wallet, not in some external classes
    {
        if (_ability)
            if (_ability.Level < _ability.MaxLevel)
